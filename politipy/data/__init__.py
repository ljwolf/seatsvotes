...
def president(year=2012):
    ...
    if isinstance(year, int):
        if year > 2012:
            raise Exception('Only presidential elections since before 2012 are stored.')
        elif year in (2012,2008):
            out = pd.read_csv(os.path.join(self_dir, 'pres_{}.csv'.format(int(year))))
        else:
        ...
        if year.lower() == 'all':
            ...
            oclass = namedtuple('presidential', ['p2012', 'p2008', 'before_2008'])
...
def canada():
    ...
    import pandas as pd
    return pd.read_csv(os.path.join(self_dir, 'canada1979.csv'))